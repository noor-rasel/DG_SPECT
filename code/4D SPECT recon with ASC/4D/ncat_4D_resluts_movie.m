...
% Im_les=postproc_cho(Im_part(:,:,8),140);
% load ncat16gtew_Im_idealUm
% Im_ideal=Im_ideal*8e6/sum(Im_ideal(:));
% Im_part=Im_ideal(23:52,16:43,29:48,:);
% Im_nor=postproc_cho(Im_part(:,:,8),140);
...
%save data_4Dmovie_clincal Im_combs
save data_4Dmovie_clincal2 Im_combs
% load data_4Dmovie_clincal Im_combs
G=16
for g=1:16 
interp_im(:,:,g)=interp2(Im_combs(:,:,g),2);
end
...
figure('position',[100 100 800 400]),movie(im,10)
% movie2avi(im,'4Dmovie_clinical')
movie2avi(im,'4Dmovie_clinical2')
