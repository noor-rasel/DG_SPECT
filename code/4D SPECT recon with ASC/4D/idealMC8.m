...
% ncatg1roiT=Im_ideal(25:49,16:40,34:38)*count_g1/sum(Im_ideal(:));
clear Im_ideal
load roi
if s==0%MC temporal only, no AC and SC: 0.04
%     gbeta=[0.01:0.01:0.07];sbeta=0;
%     for n=1:7
...
% elseif s==1%MC temporal only, AC, no SC: 0.005    
    ...
    for n=1:7
        ...
        filename=['Im8_idealMC_AC' num2str(n) '.mat'];
...
%         filename=['Im8_idealMC_ASC' num2str(n) '.mat'];
...
elseif s==1%s==3%spatial only, no AC and SC: 0.01    
    ...
    for n=1:7
        ...
        filename=['Im8_mapsnASC' num2str(n) '.mat'];
        save(filename,'Im_maps','snr_s');
    end
% elseif s==4%spatial only, AC, no SC: 0.002
%     gbeta=0;sbeta=[0.001:0.001:0.007];
%     for n=1:7
...
%         filename=['Im8_maps_AC' num2str(n) '.mat'];
...
%         filename=['Im8_maps_ASC' num2str(n) '.mat'];
