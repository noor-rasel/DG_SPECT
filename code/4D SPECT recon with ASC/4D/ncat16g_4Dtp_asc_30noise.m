function ncat16g_4Dtp_asc_30noise(com_numb)

%gbeta=[0.005:0.001:0.01 0.02:0.01:0.05];
sub_num=16;it_num=10;OF_tag=0;blur=1;
% [s,k]=ind2sub([3,30],com_numb+2);
s=1;k=1;n=com_numb+2;
% s=2;k=com_numb+1;
pause(mod(com_numb,28)*60);
%get noise sinogram
% tic,
...
% log_name=['projdata_time_n' num2str(k) '.mat'];
% projdata_time=toc;save(log_name,'projdata_time');
...
%         for n=2:5
            ...
            filename=['./data_16g_5st2/Im_ncat16g_nAS_s' num2str(n) 't' num2str(tt) '_n' num2str(k) '.mat'];
            save(filename,'Im_maps','snr_t');
%         end
    end
elseif s==2%spatial only, AC, no SC:
    gbeta=[5e-4 1e-3 2e-3];%[0.0001:0.0002:0.002];%0.001
    ...
    for tt=2%1:3
        for n=1:5
            ...
            filename=['./data_16g_5st2/Im_ncat16g_A_s' num2str(n) 't' num2str(tt) '_n' num2str(k) '.mat'];
...
elseif s==3%spatial only, AC and SC:
    ...
    for tt=2%1:3
        for n=2:5
            ...
            filename=['./data_16g_5st2/Im_ncat16g_AS_s' num2str(n) 't' num2str(tt) '_n' num2str(k) '.mat'];
