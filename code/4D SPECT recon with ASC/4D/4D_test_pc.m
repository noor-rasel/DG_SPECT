...
save noiseless_sino_attn_scat bimT bimS
...
% ncatg1roi=ncatg1(25:49,16:40,34:38)*sum(sino_p(:))/sum(ncatg1(:));
%Original phantom is not good to calculate SNR (very low)
load ncat_simind_16g_nsino.mat
sino_p=sinoP(:,:,:,1);sino_s=sinoS(:,:,:,1);clear sinoP sinoS
load Im_ideal
%Normalization: images with scatter compensation (SC) are normalized to 
%the primary count level; images without SC are normalized to the total 
%count level! If all are normalized to the same count level, images without
%SC/AC will get higher SNR than that with SC/AC. Wrong!!!
%fbp: total
ncatg1roi=Im_ideal(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_ideal(:));
load Im_fbp_tp
Im_fbp_sf=Im_fbp_sf(:,:,:,1);Im_fbp_pf=Im_fbp_pf(:,:,:,1);
Imroi=Im_fbp_sf(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_fbp_sf(:));
dsp(Imroi)
10*log10(ncatg1roi(:)'*ncatg1roi(:)/sum((ncatg1roi(:)-Imroi(:)).^2))
%fbp: primary
ncatg1roi=Im_ideal(25:49,16:40,34:38)*sum(sino_p(:))/sum(Im_ideal(:));
Imroi=Im_fbp_pf(25:49,16:40,34:38)*sum(sino_p(:))/sum(Im_fbp_pf(:));
dsp(Imroi)
10*log10(ncatg1roi(:)'*ncatg1roi(:)/sum((ncatg1roi(:)-Imroi(:)).^2))
%MAP: no AC and SC, sbeta=0.01:
%load Im_maps_sbeta001005.mat
Imroi=Im_maps001(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_maps001(:));
10*log10(ncatg1roi(:)'*ncatg1roi(:)/sum((ncatg1roi(:)-Imroi(:)).^2))
%MAP: AC only, sbeta=0.002
%load Im_atn_sca_best.mat
Imroi=Im_maps_acbest(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_maps_acbest(:));
10*log10(ncatg1roi(:)'*ncatg1roi(:)/sum((ncatg1roi(:)-Imroi(:)).^2))
dsp(Imroi)
%MAP: AC and SC, sbeta=0.003
Imroi=Im_maps_ascbest(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_maps_ascbest(:));
10*log10(ncatg1roi(:)'*ncatg1roi(:)/sum((ncatg1roi(:)-Imroi(:)).^2))
dsp(Imroi)
%MAP: no AC and SC, sbeta=0.005, gbeta=0.01
load Im_4ds0t01
Im_maps=Im_maps(:,:,:,1);
Imroi=Im_maps(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_maps(:));
10*log10(ncatg1roi(:)'*ncatg1roi(:)/sum((ncatg1roi(:)-Imroi(:)).^2))
dsp(Imroi)
%MAP: no AC and SC, sbeta=0.005, gbeta=0.01
load Im_4ds005t01
...
%for display: ideal and SC methods to primary counts; others to total counts
load ncat_simind_16g_nsino.mat
sino_p=sinoP(:,:,:,1);sino_s=sinoS(:,:,:,1);clear sinoP sinoS
load Im_ideal
ncatg1roi=Im_ideal(25:49,16:40,34:38)*sum(sino_p(:))/sum(Im_ideal(:));
...
load Im8_maps_ASC1
Im_maps=Im_maps(:,:,:,1);
Imroi=Im_maps(25:49,16:40,34:38)*sum(sino_p(:))/sum(Im_maps(:));
dsp(Imroi,1)
load Im_idealMCnAC4
Im_maps=Im_maps(:,:,:,1);
Imroi=Im_maps(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_maps(:));
...
Imroi=Im_maps(25:49,16:40,34:38)*sum(sino_p(:)+sino_s(:))/sum(Im_maps(:));
dsp(Imroi)
load Im_idealMC_ASC7
load Im8_idealMC_ASC1
Im_maps=Im_maps(:,:,:,1);
Imroi=Im_maps(25:49,16:40,34:38)*sum(sino_p(:))/sum(Im_maps(:));
dsp(Imroi,1)