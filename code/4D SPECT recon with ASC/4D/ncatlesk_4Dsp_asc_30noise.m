function ncatlesk_4Dsp_asc_30noise(com_numb)
...
if s==1%spatial only, no AC and SC:     
    ...
    for n=1:5
        ...
        filename=['./data_lesk_5st0/Im_ncatlesk_nAS_s' num2str(n) 't0_n' num2str(k) '.mat'];
        save(filename,'Im_maps','snr_s');
    end
elseif s==2%spatial only, AC, no SC: 
    gbeta=0;sbeta=[0 5e-5 1e-4 1e-3 3e-3];%[0.0001:0.0002:0.002];%0.0008
    for n=1:5
        ...
        filename=['./data_lesk_5st0/Im_ncatlesk_A_s' num2str(n) 't0_n' num2str(k) '.mat'];
...
elseif s==3%spatial only, AC and SC:
    ...
    for n=1:5
        ...
        filename=['./data_lesk_5st0/Im_ncatlesk_AS_s' num2str(n) 't0_n' num2str(k) '.mat'];
