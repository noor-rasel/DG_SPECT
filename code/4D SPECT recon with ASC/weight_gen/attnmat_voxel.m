function wp_attnwgt=attnmat_voxel(attn_map,ang,wp_vray,wp_ipxl,wp_wgt)
...
%revised Sep. 23. For non-uniform attenuation. (No symmetry!) 

[M,N,S]=size(attn_map);%detector/ray # (x) = plane # (y) = slice # (z)
FOV=64;%Use length in voxel when the unit of attenuation map is /voxel; otherwise use 40.576cm=64*.634
Total_stops=64;%revised Mingwu Jin, Sep. 23, 2006
...
for j=1:M
    wp_v=wp_vray{j};   
    if ~isempty(wp_v)
        if ang>Total_stops/2%for second half projection:%revised Mingwu Jin, Sep. 23, 2006
            wp_v=N-wp_v+1;
        end
        wp_i=wp_ipxl{j};
        wp_w=wp_wgt{j};
