...
%circular orbit only
tic
fp=fopen('wgt_p128d128s64np128_3D','r');
ndet=128;nangle=64;
nrays=ndet*nangle;
...
fclose(fp);toc
save weight128 wp_vray wp_ipxl wp_wgt
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% nplane=128;
% wp=zeros(128^2*nplane,nangle);
% for ang=1:nangle/2
%     for k=1:ndet
%         %to build first 32 planes (same distance to the detector) for each angle
%         wp_v=wp_vray{(ang-1)*128+k};%0-31
%         if ~isempty(wp_v)
%             %wp_v_max(ang,k)=max(wp_v);wp_v_min(ang,k)=min(wp_v);
%             wp_i=wp_ipxl{(ang-1)*128+k};dc=128*ones(size(wp_v));
%             iy=floor(wp_i./dc);%iy=iy+1;
%             ix=mod(wp_i,dc);ix=ix+1;
%             wp_i=iy.*dc+ix;
%             ind=wp_v*128^2+wp_i;
%             wp(ind,ang)=wp(ind,ang)+wp_wgt{(ang-1)*128+k}';
%         end
%     end
% end
% wp(wp<1e-6)=0;
% wp=reshape(wp,[128^2,128,64]);
...
save weight128_mn wp_vray wp_ipxl wp_wgt
